...
planets.map(ele=>{
    ...
    const selectPlanet=document.getElementById("select")
    selectPlanet.appendChild(options);
})
...
if(userInput==""){
    ...
    const selectedPlanet=document.getElementById("select")
    const optionIndex=selectedPlanet.selectedIndex
    const img=document.getElementById("img")
    img.src=planets[optionIndex].image
    img.style.width="90%"
    img.style.height="90%"
    img.style.opacity="80%"
    img.style.borderRadius="10px"
    const gravity=planets[optionIndex].gravity;
    const weight=gravity*userInput;
    const content1=document.getElementById("content1")
    const content2=document.getElementById("content2")
    content1.innerText=`The Weight Of An Object On ${planets[optionIndex].planetName} `
    content2.innerText=weight.toFixed(2)+"N"
}
}
